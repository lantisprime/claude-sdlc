...
| `424eed7` (PR-4) | §B2 placeholder validation. `phase-gate.sh` gains a `scan_for_placeholders` function that runs after the prior-gate exists check when the matched gate prefix is `deploy` or `fix-fast`; exits 2 listing any of the 5 required fields (signer, timestamp, work-item reference, acknowledgment, confirmation) that still contain `___`, `TODO`, or `<identifier>` placeholder tokens. Field-name match is case-insensitive; section-body extraction tracks multi-line HTML comments. §B1 prior-gate map for `build` extended to accept `fix-fast-` as an alternative to `design-`. `templates/gate.md` placeholders rewritten from multi-word descriptions to simple anchor tokens (with inline HTML-comment guidance). 10 new bats scenarios; 98/98 tests pass. |
| `d426b10` (PR-5) | §B3 file-level traceability warning. `work-item-validation.sh` now reads `CLAUDE_TOOL_INPUT.file_path` (jq → grep/sed → raw fallback) and emits two independent warn-only signals: file not in the active plan's `## In-scope files` section, or plan has no `(REQ\|TICKET\|CR\|ISSUE)-[0-9]+` reference. Generated-file inheritance via `config/tools.json`'s `generated_files` map (PR-2 schema's first consumer). In-scope check uses token-level line matching, not substring, so bare basenames don't falsely match longer paths. Existing block-level Classification + CR sign-off checks unchanged. Block-level promotion deferred to PR-8. 11 new bats scenarios (9 + 2 review-driven); 109/109 tests pass. |
...
| — (PR-7, PR-8) | Pending — Phase 3 (plan-template `## Traceability` section, then promote file-level warn to block). |
