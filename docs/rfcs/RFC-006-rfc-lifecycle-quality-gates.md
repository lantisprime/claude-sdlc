...
> Mark each PR row immediately after it merges (per AGENT-RULES.md §5 step 1). PR-5, PR-7, PR-8 still pending.

| PR / Commit | What it delivered |
|---|---|
| `2b1fbd7` (PR-1, [#41](https://github.com/lantisprime/claude-sdlc/pull/41)) | `docs/rfcs/TEMPLATE.md` Implementation-plan format reconciled to §3b. Replaces the table-based `\| Phase \| PRs / tasks \| Files in scope \|` placeholder with a `### PR-N` skeleton: **Before** / **After** / **Dependencies** / **Constraints** placeholders plus a Mermaid Sequencing example with `classDef` tier hint. Forward-only — RFC-001 and RFC-005 still use the legacy table format and were deliberately not retro-edited. |
| `769e96a` (PR-2, [#42](https://github.com/lantisprime/claude-sdlc/pull/42)) | `docs/rfcs/AGENT-RULES.md` §2–§7 each gain a `**Gate checklist:**` block (≤5 boxes per section, 28 total). Boxes cover judgment-call items the rfc-quality-gate.sh hook from PR-3 cannot machine-verify: 4-index sync, OQs closed-or-moved, `last_modified` bumps, deferral-note completeness, Implementation-table population without the `abc1234` placeholder. All boxes verifiable from the RFC plus index files only — no test or external lookup boxes. |
| `104bfca` (PR-3, [#43](https://github.com/lantisprime/claude-sdlc/pull/43)) | `.claude/hooks/rfc-quality-gate.sh` (147 lines) + 23-case bats suite. PostToolUse warn-only hook on `Edit`/`Write`/`MultiEdit`; self-filters to `*/docs/rfcs/*.md` excluding `TEMPLATE.md`, `README.md`, `pending-analysis.md`, and any file under `notes/` or `archived/`. Common checks: required `## AI context` / `## Problem` / `## Proposal` / `## Alternatives considered` headings + same-day `last_modified` bump. Status-driven checks: `accepted` requires `**Decision:** proceed` and a `### PR-N` plan; `implemented` requires a non-`abc1234` Implementation row; `deferred` / `withdrawn` / `superseded` require a populated note section. Stdin parsed without `jq` (grep+sed); pipe-failure-safe via `\|\| true`. |
| `d6b7ba4` (PR-4, [#44](https://github.com/lantisprime/claude-sdlc/pull/44)) | `.claude/hooks/ai-slop-check.sh` (112 lines) + 17-case bats suite. PostToolUse warn-only hook on `*.md` under `docs/` or `rfcs/`. Closed six-pattern set frozen by RFC (adding a seventh requires a follow-up RFC, not a quiet hook edit): inflated metaphors, manufactured persona, formulaic triplets, false-severity escalation, unsupported compliance assertions, aspirational framing. All scans use `grep -inE` (case-insensitive extended regex) so capitalised variants are caught. Output: `[ai-slop] WARN: <file>:<line>: <class> — "<match>"` followed by `[ai-slop] PROPOSED FIX: …`. Never auto-applies — proposals only. |
| `3e4a517` (PR-6, [#45](https://github.com/lantisprime/claude-sdlc/pull/45)) | `.claude/agents/rfc-pr-reviewer.md` (85 lines) — Haiku 4.5 plan-conformance review agent. Model pinned by exact ID (`claude-haiku-4-5-20251001`), not alias, to defend against silent model upgrades. Four checks: scope match (PR files ⊆ files listed in the §3b `### PR-N` block + their test counterparts), surgical-edit (`git diff --function-context` hunk headers must not span functions outside plan), AI-slop on prose changes (same closed pattern set as PR-4), test coverage. Bounded write scope declared in frontmatter — stdout only; no `gh pr review` submission, no source/test/config edits, no agent re-spawning. Maintainer-only; plugin `agents=5` count unchanged. |
